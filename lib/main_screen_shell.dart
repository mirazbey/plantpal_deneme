// lib/main_screen_shell.dart (BAŞTAN SONA EKSİKSİZ VE HATASIZ FİNAL KOD)
...
class MainScreenShellState extends State<MainScreenShell> with SingleTickerProviderStateMixin {
  ...
  // lib/main_screen_shell.dart dosyasındaki build metodunu bununla değiştirin
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        leading: _isIdentifying
            ? IconButton(
                ...
                onPressed: _cancelIdentification,
              )
            : (_pageIndex == 0 ? _buildFlashingTextBubble(
        ...
        title: Text(
          ...
          _isIdentifying && _predictions.isEmpty ? '' : // Animasyon varken başlık boş
        ...
        actions: [
          if (_isIdentifying && _predictions.isNotEmpty)
            IconButton(
      ...
      body: _isIdentifying
          ? (_predictions.isEmpty
              // Yükleme devam ediyorsa animasyonlu ekranı göster
              ? AnimatedIdentificationLoader(selectedImage: _selectedImage!)
              // Yükleme bittiyse ve sonuçlar varsa sonuç sayfasını göster
              // lib/main_screen_shell.dart -> build metodu -> body: ...

            : IdentifyPage(
                selectedImage: _selectedImage,
                // DÜZELTME: Eksik isLoading parametresi eklendi
                isLoading: _predictions.isEmpty, // Tahminler boşsa, hala yüklüyor demektir
          ...
          // Tanımlama modu aktif değilse normal sayfaları göster
  ...
  Future<void> _processImageIdentification(ImageSource source) async {
    ...
    // Arka planda işlemleri yap (artık arayüzü adım adım güncellemiyoruz)
    ...
    // Sonucu işle ve sonuç sayfasına geç
    if (result != null && result.contains('---TAHMİN')) {
      ...
      // Hata durumunda, belki bir hata mesajı gösterip süreci iptal edebiliriz
      _cancelIdentification(); // Şimdilik iptal edelim
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(result ?? 'Tanımlama başarısız oldu.')),
      );
    }
  }
  ...
  final TimeOfDay? pickedTime = await showTimePicker(
      context: currentContext, initialTime: TimeOfDay.now());
